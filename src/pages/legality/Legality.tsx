...
export default function Legality() {
  ...
  const getStatus = (progress: number) => {
    if (progress === 100)
      return {
        key: "completed",
        label: t("legality.completed"),
        ...statusMap.completed,
      };
    if (progress >= 50)
      return {
        key: "inProgress",
        label: t("legality.inProgress"),
        ...statusMap.inProgress,
      };
    if (progress >= 25)
      return {
        key: "started",
        label: t("legality.started"),
        ...statusMap.started,
      };
    return {
      key: "pending",
      label: t("legality.pending"),
      ...statusMap.pending,
    };
  };

  const fmtDate = (d: string) =>
    new Date(d).toLocaleDateString(i18n.language === "ar" ? "ar-SA" : "en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
  ...
  return (
    <Shell>
      ...
      <DeleteDialog
        ...
        onConfirm={() =>
          legalityToDelete && deleteMutation.mutate(legalityToDelete)
        }
        ...
        <div className="max-w-[1560px] mx-auto px-6 lg:px-10 py-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              <Link href="/legality/new">
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="text-[13px] font-semibold text-[#1C1917] dark:text-white">
                      {t("legality.newLegality")}
                    </p>
                    <p className="text-[11px] text-[#A8A29E] mt-0.5">
                      {t("legality.startWorkflow")}
                    </p>
                  </div>
                </motion.div>
              </Link>
            ...
            {isLoading &&
              Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-[#18181B] rounded-md border border-black/[0.06] dark:border-white/[0.06] p-5 space-y-4 animate-pulse"
                >
              ...
              data?.data.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "h-[3px] w-full bg-gradient-to-r shrink-0",
                        status.gradient,
                      )}
                    />
                    ...
                    <div className="p-5 flex flex-col gap-3 flex-1">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        >
                          <span
                            className={cn("w-1.5 h-1.5 rounded-md", status.dot)}
                          />
                          {status.label}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-md overflow-hidden">
                          <motion.div
                            ...
                            className={cn(
                              "h-full rounded-md bg-gradient-to-r",
                              status.gradient,
                            )}
                          />
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          <Can I="UPDATE" a="legality">
                            <Link href={`/legality/${item.id}/edit`}>
                              <button
                                className="w-8 h-8 rounded-md border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:border-[#B39371] hover:text-[#B39371] transition-colors"
                                title={t("common.edit")}
                              >
          ...
          {!isLoading && data?.data.length === 0 && (
            ...
            >
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-[#1C1917] dark:text-white">
                  {t("legality.noLegalities")}
                </h3>
                <p className="text-[12px] text-[#A8A29E] mt-1 max-w-xs">
                  {searchValue || filterStatus !== "all"
                    ? t("legality.adjustSearch")
                    : t("legality.getStarted")}
                </p>
              </div>
              {!searchValue && filterStatus === "all" && (
                <Link href="/legality/new">
