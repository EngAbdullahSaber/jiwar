...
export function UpdateUserDialog({
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.message?.english || t("users.errorUpdate"),
      );
    },
  });
            ...
            >
              ...
              <div className="px-8 -mt-6 relative z-20">
                <div className="bg-white dark:bg-gray-900 rounded-md p-4 shadow-xl border border-gray-100 dark:border-gray-800 flex items-center gap-4">
                  ...
                  <div className="text-right rtl:text-left">
                    ...
                    <p className="text-xs font-mono font-bold text-[#B39371]">
              ...
              <form onSubmit={handleSubmit} className="p-8 pt-6 space-y-6">
                <div className="space-y-5">
                  <FormField label={t("users.emailAddress")} required error={errors.email}>
                    <div className="relative group">
                      ...
                      <Input id="email" name="email" type="email" placeholder={t("users.placeholders.email")} className="h-12 pl-12 rtl:pl-4 rtl:pr-12 rounded-md bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 focus:bg-gray-50 dark:focus:bg-gray-800 focus:ring-4 focus:ring-[#B39371]/10 focus:border-[#B39371]/50 transition-all font-semibold text-gray-700 dark:text-gray-200" value={formData.email} onChange={(e) => { setFormData((prev) => ({ ...prev, email: e.target.value })); if (errors.email) setErrors((p) => { const { email, ...r } = p; return r; }); }} />
                    </div>
                  </FormField>

                  <FormField label={t("users.phoneNumber")} error={errors.phoneNumber}>
                    <div className="relative group">
                      ...
                      <Input id="phoneNumber" name="phoneNumber" type="tel" placeholder={t("users.placeholders.phoneNumber")} className="h-12 pl-12 rtl:pl-4 rtl:pr-12 rounded-md bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 focus:bg-gray-50 dark:focus:bg-gray-800 focus:ring-4 focus:ring-[#B39371]/10 focus:border-[#B39371]/50 transition-all font-semibold text-gray-700 dark:text-gray-200" value={formData.phoneNumber} onChange={(e) => { setFormData((prev) => ({ ...prev, phoneNumber: e.target.value })); if (!e.target.value || phoneRegex.test(e.target.value)) setErrors((p) => { const { phoneNumber, ...r } = p; return r; }); }} />
                    </div>
                  </FormField>

                  <FormField label={t("users.role")} required error={errors.roleId}>
                    <PaginatedSelect
                      ...
                      mapResponseToOptions={(res) =>
                        res.data.filter((role: any) => role.id !== 5 && role.id !== 6).map((role: any) => ({ value: role.id.toString(), label: role.name, description: role.description, icon: <Shield className="w-4 h-4" /> }))
                      }
                    />
                  </FormField>
                </div>
