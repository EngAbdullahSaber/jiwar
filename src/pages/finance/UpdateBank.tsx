...
export default function UpdateBank() {
  ...
  useEffect(() => {
    if (bankData?.data) {
      ...
      setFormData({
        name: {
          arabic: bank.name?.arabic || "",
          english: bank.name?.english || ""
        },
        countryId: bank.country?.id?.toString() || ""
      });
    }
  }, [bankData]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const response = await api.patch(`/bank/${id}`, payload);
      return response.data;
    },
    onSuccess: () => {
      toast.success(t('banks.updateSuccess'), {
        icon: '🏦',
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 pb-32">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <FormSection 
              ...
              description={t('banks.identityLocationEditDesc')}
              delay={0.1}
            >
