...
export default function UpdateTemplate() {
  ...
  const { data: templateData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const resp = await api.get(`/template/${id}`);
  ...
  const mutation = useMutation({
    mutationFn: async (values: TemplateFormValues) => {
      const response = await api.patch(`/template/${id}`, values);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <SelectValue
                        placeholder={t("templates.placeholders.selectModel")}
                      />
                    </SelectTrigger>
                    <SelectContent className="rounded-md border-gray-200 dark:border-gray-700">
                      {["Residential", "Commercial", "Mixed-use", "Type A"].map(
                        (type) => (
                          <SelectItem
                            key={type}
                            value={type}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <FormField
                    ...
                    error={errors.clothsRoom?.message}
                  >
                    <Input
                      type="number"
                      step="0.01"
                  ...
                  <FormField
                    ...
                    error={errors.driverRoom?.message}
                  >
                    <Input
                      type="number"
                      step="0.01"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <FormField
                    label={`${t("templates.size")} (${t("templates.sqm")})`}
                    required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormField label={t("templates.amenities.balcony")}>
                    <RadioGroup
                      defaultValue={
                        templateData?.balconyAccess ? "true" : "false"
                      }
                      onValueChange={(val) =>
                        setValue("balconyAccess", val === "true")
                      }
                    ...
                    >
                      {["Yes", "No"].map((option) => {
                        ...
                        const label =
                          option === "Yes"
                            ? t("common.yes" as any) || "Yes"
                            : t("common.no" as any) || "No";
                        return (
                  ...
                  <FormField label={t("templates.location")}>
                    <RadioGroup
                      defaultValue={templateData?.location || "FRONT"}
                      onValueChange={(val) =>
                        setValue("location", val as "FRONT" | "BACK")
                      }
                      className="flex gap-3"
                    >
                      {["FRONT", "BACK"].map((loc) => (
                ...
                <FormField label={t("templates.sections.features")}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                      {
                        key: "duelEntrances",
                        label: t("templates.amenities.dualEntrances"),
                        icon: "🚪",
                      },
                      {
                        key: "familyLounge",
                        label: t("templates.amenities.familyLounge"),
                        icon: "🛋️",
                      },
                      {
                        key: "guestMajlis",
                        label: t("templates.amenities.guestMajlis"),
                        icon: "🕌",
                      },
                      {
                        key: "kitchen",
                        label: t("templates.amenities.kitchen"),
                        icon: "🍳",
                      },
                      {
                        key: "rooftop",
                        label: t("templates.amenities.rooftop"),
                        icon: "🏠",
                      },
                    ].map((feature) => (
                      <motion.div
