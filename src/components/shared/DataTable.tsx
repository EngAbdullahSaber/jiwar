...
export function DataTable<T extends Record<string, any>>({
  ...
  const getSortIcon = (column: Column<T>) => {
    ...
    return (
      <ChevronDown className="w-3.5 h-3.5 ml-1 opacity-0 group-hover:opacity-30" />
    );
  };
              ...
              {columns.map((column, index) => (
                <th
                  ...
                  className={cn(
                    ...
                    column.sortable &&
                      "cursor-pointer hover:text-[#4A1B1B] dark:hover:text-[#B39371]",
